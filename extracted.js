...
function qcInputPrompt(label,value,type){return new Promise(function(resolve){
  ...
  setTimeout(()=>{input.focus(); if(type==='date'){try{if(input.showPicker)input.showPicker()}catch(e){}}},60);
});}
...
if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=20260917-7').catch(()=>{}));}
...
(function(){
  ...
  function classifyDeltaStacks(stackText){
    const ws=[],rs=[],wmt={},rmt={};
    String(stackText||'').split(',').map(v=>v.trim()).filter(Boolean).forEach(st=>{
      const rr=rows.find(row=>String(stack(row)||'').trim()===st);
      if(rr){if(isWheat(rr)){if(!ws.includes(st))ws.push(st);wmt[st]=chemNum(qty(rr));}else if(isRice(rr)){if(!rs.includes(st))rs.push(st);rmt[st]=chemNum(qty(rr));}}
      else {if(!ws.includes(st)&&!rs.includes(st))ws.push(st);}
    });
    return {wheatStacks:ws,riceStacks:rs,wheatMt:ws.reduce((s,st)=>s+chemNum(wmt[st]),0),riceMt:rs.reduce((s,st)=>s+chemNum(rmt[st]),0)};
  }
  function deltaAutoGroups(){
    const groups={};
    rows.filter(r=>qty(r)>0 && val(r,'Fumigation Date')).forEach(r=>{
      const fum=String(val(r,'Fumigation Date')).trim();
      const entry=deltaDateText(fum); if(!entry)return;
      if(!groups[entry])groups[entry]={date:entry,fumDate:fum,plannedDate:entry,stacks:[],wheat:0,rice:0,wheatMt:0,riceMt:0};
    ...
    Object.keys(overrides).forEach(k=>{const ov=overrides[k];if(!ov||!ov.locked)return;if(!groups[k])groups[k]={date:k,fumDate:ov.fumDate||'',plannedDate:ov.plannedDate||k,stacks:[],wheat:0,rice:0,wheatMt:0,riceMt:0};});
    return Object.values(groups).map(g=>{
      const ov=overrides[g.date];
      if(ov?.locked){
        const st=String(ov.stackText??'');
        const cls={wheatStacks:Array.isArray(ov.wheatStacks)?ov.wheatStacks:[],riceStacks:Array.isArray(ov.riceStacks)?ov.riceStacks:[],wheatMt:chemNum(ov.wheatMt),riceMt:chemNum(ov.riceMt)};
        return {...g,...ov,stackText:st,count:Number(ov.count??(st?st.split(',').filter(Boolean).length:0)),wheat:Number(ov.wheat??cls.wheatStacks.length),rice:Number(ov.rice??cls.riceStacks.length),wheatStacks:cls.wheatStacks,riceStacks:cls.riceStacks,wheatMt:cls.wheatMt,riceMt:cls.riceMt,consumption:chemNum(ov.consumption),locked:true};
      }
      const stackText=g.stacks.join(', '), cls=classifyDeltaStacks(stackText);
      return {...g,...(ov||{}),stackText:ov?.stackText!=null?ov.stackText:stackText,count:g.stacks.length,wheat:Number(g.wheat||0),rice:Number(g.rice||0),wheatStacks:cls.wheatStacks,riceStacks:cls.riceStacks,wheatMt:cls.wheatMt,riceMt:cls.riceMt,consumption:g.stacks.length*0.650,locked:false};
    }).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  }
  function deltaTableRows(){const a=deltaAutoGroups();let progressive=0;return a.length?a.map((x,i)=>{const isLocked=!!x.locked;progressive+=chemNum(x.consumption);const ws=x.wheatStacks||[],rs=x.riceStacks||[];const action=isLocked?`<div class="chem-action-group"><span class="alp-lock-badge">🔒 Locked</span><button class="pill chem-edit-btn" onclick="editDeltaAuto('${escC(x.date)}')">✏️ Edit</button><button class="pill chem-delete-btn" onclick="deleteDeltaAuto('${escC(x.date)}')">🗑 Delete</button></div>`:`<div class="chem-action-group"><button class="pill chem-save-btn" onclick="saveDeltaAuto('${escC(x.date)}')">💾 Save</button></div>`;return `<tr class="${isLocked?'alp-row-locked':''}"><td>${i+1}</td><td>${chemDate(x.date)}</td><td><div class="delta-stack-split"><div class="delta-wheat-stacks">${ws.map(st=>`<span>${escC(st)}</span>`).join('')||'—'}</div><div class="delta-rice-stacks">${rs.map(st=>`<span>${escC(st)}</span>`).join('')||'—'}</div></div></td><td>${chemNum(x.consumption).toFixed(3)} kg</td><td><b>${progressive.toFixed(3)} kg</b></td><td>${ws.length} Wheat / ${rs.length} Rice</td><td>${action}</td></tr>`;}).join(''):'<tr><td colspan="7" class="empty">No fumigated stacks found.</td></tr>';}
  ...
  window.saveDeltaAuto=function(dateKey){const g=deltaAutoGroups().find(x=>x.date===dateKey);if(!g)return;let o={};try{o=JSON.parse(localStorage.getItem('shambhuDeltaOverridesV1')||'{}')}catch(e){};const old=o[g.date]&&o[g.date].locked?o[g.date]:{};const cls=classifyDeltaStacks(g.stackText||g.stacks.join(', '));o[g.date]={...old,date:g.date,fumDate:g.fumDate||old.fumDate||'',plannedDate:g.plannedDate||g.date,stackText:String(g.stackText??old.stackText??''),count:Number(g.count??old.count??0),wheat:cls.wheatStacks.length,rice:cls.riceStacks.length,wheatStacks:cls.wheatStacks,riceStacks:cls.riceStacks,wheatMt:cls.wheatMt,riceMt:cls.riceMt,consumption:old.consumption!=null?chemNum(old.consumption):chemNum(g.count??old.count??0)*0.650,locked:true,sourceLockedAt:old.sourceLockedAt||new Date().toISOString()};localStorage.setItem('shambhuDeltaOverridesV1',JSON.stringify(o));const r=document.getElementById('chemRows');if(r)r.innerHTML=deltaTableRows();renderChemicalReport();};
  window.editDeltaAuto=async function(dateKey){const pin=await qcPinPrompt('Edit ke liye PIN enter karein');if(pin===null||String(pin)!==String(getSitePin())){if(pin!==null)alert('Wrong PIN.');return;}const g=deltaAutoGroups().find(x=>x.date===dateKey);if(!g)return;const newDate=await qcDatePrompt('Delta Spray Date',g.date);if(newDate===null||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(newDate))return;const stacks=await qcInputPrompt('Stack Number(s) — comma separated',g.stackText||[...(g.wheatStacks||[]),...(g.riceStacks||[])].join(', '),'text');if(stacks===null)return;const cons=await qcInputPrompt('Consumption (kg)',chemNum(g.consumption).toFixed(3),'number');if(cons===null)return;let o={};try{o=JSON.parse(localStorage.getItem('shambhuDeltaOverridesV1')||'{}')}catch(e){};const cls=classifyDeltaStacks(stacks);const prior=o[dateKey]&&o[dateKey].locked?o[dateKey]:{};if(newDate!==dateKey)delete o[dateKey];o[newDate]={...prior,date:newDate,fumDate:prior.fumDate||g.fumDate||'',plannedDate:prior.plannedDate||g.plannedDate||newDate,stackText:String(stacks),count:cls.wheatStacks.length+cls.riceStacks.length,wheat:cls.wheatStacks.length,rice:cls.riceStacks.length,wheatStacks:cls.wheatStacks,riceStacks:cls.riceStacks,wheatMt:cls.wheatMt,riceMt:cls.riceMt,consumption:chemNum(cons),locked:true,sourceLockedAt:prior.sourceLockedAt||new Date().toISOString()};localStorage.setItem('shambhuDeltaOverridesV1',JSON.stringify(o));const r=document.getElementById('chemRows');if(r)r.innerHTML=deltaTableRows();renderChemicalReport();};
